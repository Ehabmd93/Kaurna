...
def track_mixes_and_marsh_values(data):
    mix_a_count = mix_b_count = mix_c_count = mix_d_count = 0
    error_list = []
    mix_a_marsh = mix_b_marsh = mix_c_marsh = mix_d_marsh = None
    mix_change_time = None
    current_mix = 'A'
    last_mix_change_time = None
    marsh_update_error_reported = False
    ...
    for i, row in data.iterrows():
        ...
        if mix_change_time is None:
            mix_change_time = timestamp

        if current_mix == 'A':
            if (timestamp - mix_change_time).total_seconds() / 60 >= 10:
                if mix_a_marsh is None:
                    mix_a_marsh = marsh_value
                    mix_a_count += 1
                elif marsh_value != mix_a_marsh:
                    mix_a_count += 1
                    mix_a_marsh = marsh_value
            if mix_num == 3:
                mix_change_time = timestamp
                current_mix = 'B'
                if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                    mix_b_marsh = marsh_value
                    mix_b_count += 1
        elif current_mix == 'B':
            if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                if mix_b_marsh is None:
                    mix_b_marsh = marsh_value
                    mix_b_count += 1
                elif marsh_value != mix_b_marsh:
                    mix_b_count += 1
                    mix_b_marsh = marsh_value
            if mix_num == 4:
                mix_change_time = timestamp
                current_mix = 'C'
                if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                    mix_c_marsh = marsh_value
                    mix_c_count += 1
        elif current_mix == 'C':
            if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                if mix_c_marsh is None:
                    mix_c_marsh = marsh_value
                    mix_c_count += 1
                elif marsh_value != mix_c_marsh:
                    mix_c_count += 1
                    mix_c_marsh = marsh_value
            if mix_num == 5:
                mix_change_time = timestamp
                current_mix = 'D'
                if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                    mix_d_marsh = marsh_value
                    mix_d_count += 1
        elif current_mix == 'D':
            if (timestamp - mix_change_time).total_seconds() / 60 >= 7:
                if mix_d_marsh is None:
                    mix_d_marsh = marsh_value
                    mix_d_count += 1
                elif marsh_value != mix_d_marsh:
                    mix_d_count += 1
                    mix_d_marsh = marsh_value
        
        if last_mix_change_time is not None and (timestamp - last_mix_change_time).total_seconds() / 60 <= 5:
            continue

        if not marsh_update_error_reported:
            if (timestamp - mix_change_time).total_seconds() / 60 > 10 and current_mix == 'A':
                error_list.append("GIV operator didn't update Marsh value on time")
                marsh_update_error_reported = True
            elif (timestamp - mix_change_time).total_seconds() / 60 > 7 and current_mix in ['B', 'C', 'D']:
                error_list.append("GIV operator didn't update Marsh value on time")
                marsh_update_error_reported = True
...
def update_injection_details(data, selected_stage, selected_hole_id):
    try:
        ...
        details = [
            ...
            html.B("Hole ID: "), html.Span(f"{selected_hole_id}", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Stage: "), html.Span(f"{selected_stage}", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Commencement Date/ Time: "), html.Span(f"{first_date} at {first_time}", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Completion Date/ Time: "), html.Span(f"{last_date} at {last_time}", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Stage Top: "), html.Span(f"{stage_top} meters", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Stage Bottom: "), html.Span(f"{stage_bottom} meters", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.H3("Net Mixes Volumes:"),
            html.B(f"Mix A: "), html.Span(f"{mix_a_volume:.2f} L", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B(f"Mix B: "), html.Span(f"{mix_b_volume:.2f} L", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B(f"Mix C: "), html.Span(f"{mix_c_volume:.2f} L", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B(f"Mix D: "), html.Span(f"{mix_d_volume:.2f} L", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B(f"Cumulative Volume: "), html.Span(f"{cumulative_volume:.2f} L", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Total Grouting Time: "), html.Span(f"{total_grouting_time:.2f} hours", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
            html.B("Net Grouting Time: "), html.Span(f"{net_grouting_time:.2f} hours", style={'color': 'green', 'font-weight': 'bold'}),
            html.Br(),
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Label('Hole ID'),
            dcc.Dropdown(id='hole-id-dropdown', placeholder="Select a Hole ID")
        ...
        html.Div([
            html.Label('Stage'),
            dcc.Dropdown(id='stage-dropdown', placeholder="Select a Stage")
...
def update_dropdowns(filename):
    ...
    hole_id_options = [{'label': hole_id, 'value': hole_id}] if hole_id else []
    order_options = [{'label': order, 'value': order}] if order else []
    stage_options = [{'label': stage, 'value': stage}] if stage else []
    
    return hole_id_options, order_options, stage_options
...
def update_and_run_tool(contents, n_clicks, selected_hole_id, selected_order, selected_stage, view_type, filename):
    ...
    if trigger_id == 'upload-data' and contents is not None:
        try:
            ...
            injection_details = update_injection_details(data, selected_stage, selected_hole_id)
            ...
            error_summary = html.Div([html.Span(error, style={'color': 'red'}) for error in set(mixes_and_marsh['Errors'])])  # Using set to remove duplicates
    ...
    elif trigger_id in ['run-tool-button', 'hole-id-dropdown', 'order-dropdown', 'stage-dropdown', 'view-type']:
        if contents is None:
        ...
        # Add additional logic for updating the graph based on selected options if necessary
